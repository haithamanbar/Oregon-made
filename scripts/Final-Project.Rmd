...
In this research, we ran a regression model on attendance data and proficiency data of schools in Oregon. We obtained two different data sets from the Oregon Department of Education (ODE): attendance rate by school and student group, and proficiency in English, Language, and Arts by student group. 
...
A simple regression analysis was run to examine the relationship between the percentage of regular attenders and the percentage of proficient students in Oregon schools. A total of `r school_num` Oregon schools with existing data were included in the sample.

_Predicted percentage of proficient students = 46.5 + 0.423 (percentage of regular attendance)_
...
The relationship as estimated by ordinary-least-squares regression has a slope of 0.426  and a p-value of 0.013 making us to fail to reject the null hypothesis that there is no relationship between student school attendance and proficiency rate in ELA on average in the population . 

#Statistical assumptions

```{r}
attpro_residuals<-residuals(attpro)
summary(attpro_residuals)
sd(attpro_residuals)
```

```{r}
attpro$predict <- predict(attpro)
attpro$resid <- residuals(attpro)

summary(attpro$resid)
sd(attpro$resid)

boxplot(resid(attpro))

hist(attpro_residuals)

```

From the analysis of the residuals as fitted in the model, it shows that the regression assumptions have been reasonably met hence appropriately characterizing student school attendance to proficiency rate in ELA. Our residuals are symmetrically distributed around 0.
...
plot4 <- attend_race %>%
  ...
  theme_bw() +
  labs(title="Percent proficient by race",
       x="Race",
...
plot3 <- attend_race %>%
   ...
   geom_boxplot(fill = "skyblue", alpha = 0.5) +
  theme_bw() +
  labs(title="Percent regular attendance by race",
       x="Race",
       y="Percent regular attendance (%)",
       tag="Plot 4") +
  coord_flip()
plot3
```
