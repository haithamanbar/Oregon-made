...
race <- import(here("data", "pagr_schools_ela_tot_raceethnicity_2122.csv")) %>% 
  clean_names() %>%
  as_tibble()
view(race) #I think one should avoid using View() in an rmd/R script because other people using the script and running the code may find it odd when new tabs open while the code is running
...
attend1 <- attend %>% 
   filter(student_group == "American Indian/Alaska Native" | # one suggestion I have for shorter, less redundant code would be to use filter(student_group %in% c("Asian", "Black/African American", "Hispanic/Latino", "Multi-Racial", "Native Hawaiian/Pacific Islander", "White"))
          student_group == "Asian"|
...
prof_percent_by_type_of_inst %>%
  kable(caption="Student Proficiency Rate by Race and Institution types", #I never thought I could get excited about a table, but this looks amazingly clean!
...
attend_race %>%
  ...
  geom_point() +
  geom_smooth(method='lm') + # this is not showing because percent_regular_attenders is a character variable
  theme_bw() +
  labs(title="Student Attendance and Proficiency Rate \n in English, Language, and Arts",
       ...
       tag = "Plot 1") #this is great, I did not know about the tag argument!

#Adding plot for attendance vs. proficiency, faceted by race
attend_race %>%
  ggplot(aes(x=percent_regular_attenders, y=percent_proficient,group=student_group)) + #This plot looks great!
  geom_point() +
