...
```{r attendance-table, results='markup'}
...
#General table for attendance by race
attend_by_race <- attend_race %>%
    group_by(institution_type, student_group) %>% 
...
absent_by_race_table <- attend_by_race  %>%
  select(-number_chronically_absent, -number_regular_attenders, -students_included, -percent_regular_attenders) %>% 
  pivot_wider(
    names_from = "student_group",
    values_from = "percent_chron_absent") %>% 
  clean_names() 


absent_by_race_table %>%
  kable(caption="Attendance by race",
        col.names=c("Institution type",
                    "Asian", 
                    "Black African American", 
                    "Hispanic/Latino",
...
pivot_longer(
  cols = 1:6,
  names_to = "race",
  values_to = "percent_chron_absent") 
