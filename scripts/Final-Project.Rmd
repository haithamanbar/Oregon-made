...
race1 <- race %>% 
 ...
 filter(number_proficient != "*" &
        number_proficient != "--") %>% 
  ## I have struggled to change variables to numeric. I never knew about Transform(). This is helpful. 
...
attend_race %>%
  ## No need for [x=, y=] when piping. 
  ggplot(aes(percent_regular_attenders, percent_proficient)) +
  geom_point() +
...
attend_race %>%
  ggplot(aes(percent_regular_attenders, percent_proficient,group=student_group)) +
  geom_point() +
  geom_smooth(method='lm') +  
